...
void MainThread::search() {
  ...
  else if (relative_rank(WHITE, rootPos.square<KING>(WHITE)) >= RANK_5 || relative_rank(BLACK, rootPos.square<KING>(BLACK)) >= RANK_5)
  {
	  rootMoves.emplace_back(MOVE_NONE);
	  sync_cout << "info depth 0 score "
...
moves_loop: // When in check, search starts from here
    ...
    */
	...
	else if(relative_rank(WHITE, pos.square<KING>(WHITE)) >= RANK_5 || relative_rank(BLACK, pos.square<KING>(BLACK)) >= RANK_5)
		bestValue = excludedMove ? alpha
		           :  mated_in(ss->ply);
    else if (bestMove)
    {
